        ...
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div class="flex items-center space-x-3 flex-wrap gap-2">
                ...
                <a href="{{ route('admin.renter.index', ['status' => 'pending']) }}"
                   class="px-4 py-2 text-sm font-medium {{ request('status') == 'pending' ? 'text-yellow-700 bg-yellow-50 border-2 border-yellow-400' : 'text-gray-700 bg-white border border-gray-200' }} rounded-lg hover:bg-yellow-100 transition-colors">
                    PENDING
                </a>
                <a href="{{ route('admin.renter.index', ['status' => 'confirmed']) }}"
                   class="px-4 py-2 text-sm font-medium {{ request('status') == 'confirmed' ? 'text-blue-700 bg-blue-50 border-2 border-blue-400' : 'text-gray-700 bg-white border border-gray-200' }} rounded-lg hover:bg-blue-100 transition-colors">
                    CONFIRMED
                </a>
                <a href="{{ route('admin.renter.index', ['status' => 'running']) }}"
                   class="px-4 py-2 text-sm font-medium {{ request('status') == 'running' ? 'text-purple-700 bg-purple-50 border-2 border-purple-400' : 'text-gray-700 bg-white border border-gray-200' }} rounded-lg hover:bg-purple-100 transition-colors">
                    RUNNING
                </a>
                <a href="{{ route('admin.renter.index', ['status' => 'completed']) }}"
                   class="px-4 py-2 text-sm font-medium {{ request('status') == 'completed' ? 'text-green-700 bg-green-50 border-2 border-green-400' : 'text-gray-700 bg-white border border-gray-200' }} rounded-lg hover:bg-green-100 transition-colors">
                    COMPLETED
                </a>
                <a href="{{ route('admin.renter.index', ['status' => 'cancelled']) }}"
                   class="px-4 py-2 text-sm font-medium {{ request('status') == 'cancelled' ? 'text-red-700 bg-red-50 border-2 border-red-400' : 'text-gray-700 bg-white border border-gray-200' }} rounded-lg hover:bg-red-100 transition-colors">
                    CANCELLED
                </a>
            </div>

            <div class="flex items-center gap-2">
                <button onclick="window.print()" class="flex items-center px-4 py-2 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 text-sm font-medium text-gray-700 transition-all">
