...
<x-admin-layout>
    ...
    <style>
        ...
        .rp * { font-family: 'Plus Jakarta Sans', sans-serif; box-sizing: border-box; }
        ...
        .page-hero::before { content:''; position:absolute; top:-50px; right:-50px; width:180px; height:180px; background:rgba(251,191,36,0.10); border-radius:50%; }
        .page-hero::after  { content:''; position:absolute; bottom:-30px; left:38%; width:120px; height:120px; background:rgba(251,191,36,0.06); border-radius:50%; }
        .hero-top { display:flex; align-items:center; justify-content:space-between; margin-bottom:1.25rem; position:relative; z-index:1; gap:1rem; flex-wrap:wrap; }
        .hero-title { color:#fff; font-size:1.5rem; font-weight:700; letter-spacing:-0.02em; }
        .hero-title span { background:linear-gradient(90deg,#fbbf24,#f59e0b); -webkit-background-clip:text; -webkit-text-fill-color:transparent; }
        .hero-sub { color:rgba(255,255,255,0.40); font-size:0.78rem; margin-top:3px; }
        ...
        .btn-add {
            ...
            box-shadow:0 3px 10px rgba(245,158,11,0.3); transition:all 0.2s;
        }
        .btn-add:hover { background:#d97706; transform:translateY(-1px); }

        .stat-grid { display:grid; grid-template-columns:repeat(5,1fr); gap:0.75rem; position:relative; z-index:1; }
        .stat-card { background:rgba(255,255,255,0.07); border:1px solid rgba(255,255,255,0.10); border-radius:12px; padding:0.85rem 1rem; }
        .stat-label { font-size:0.6rem; font-weight:700; text-transform:uppercase; letter-spacing:0.08em; color:rgba(255,255,255,0.40); margin-bottom:4px; display:flex; align-items:center; gap:5px; }
        .stat-dot { width:6px; height:6px; border-radius:50%; display:inline-block; flex-shrink:0; }
        .stat-val { font-size:1.6rem; font-weight:700; color:#fff; line-height:1; }

        /* ── TOAST ── */
        .toast-ok { background:#f0fdf4; border:1px solid #bbf7d0; border-radius:12px; padding:0.8rem 1rem; display:flex; align-items:center; justify-content:space-between; margin-bottom:1rem; font-size:0.82rem; color:#166534; }

        /* ── FILTER BAR ── */
        .filter-bar { background:#fff; border-radius:14px; border:1px solid #e5e7eb; padding:0.85rem 1.1rem; margin-bottom:1.1rem; display:flex; align-items:center; gap:0.75rem; flex-wrap:wrap; }
        .search-wrap { position:relative; flex:1; min-width:180px; }
        .search-wrap input { width:100%; padding:0.5rem 0.8rem 0.5rem 2.2rem; border:1.5px solid #e5e7eb; border-radius:9px; font-size:0.8rem; outline:none; color:#111827; font-family:inherit; transition:border-color 0.15s; }
        .search-wrap input:focus { border-color:#f59e0b; box-shadow:0 0 0 3px rgba(245,158,11,0.10); }
        .search-wrap .si { position:absolute; left:8px; top:50%; transform:translateY(-50%); width:14px; height:14px; color:#9ca3af; pointer-events:none; }
        .search-wrap .cx { position:absolute; right:8px; top:50%; transform:translateY(-50%); color:#9ca3af; line-height:1; }
        .search-wrap .cx:hover { color:#374151; }

        .pills { display:flex; gap:5px; flex-wrap:wrap; }
        .pill { padding:4px 11px; border-radius:20px; font-size:0.7rem; font-weight:600; border:1.5px solid #e5e7eb; background:#f9fafb; color:#6b7280; text-decoration:none; white-space:nowrap; transition:opacity 0.15s; }
        ...
        .date-sel { padding:0.5rem 0.8rem; border:1.5px solid #e5e7eb; border-radius:9px; font-size:0.78rem; color:#374151; outline:none; background:#fff; cursor:pointer; font-family:inherit; transition:border-color 0.15s; }
        .date-sel:focus { border-color:#f59e0b; }

        /* ── DESKTOP TABLE ── */
        .table-wrap { background:#fff; border-radius:14px; border:1px solid #e5e7eb; overflow:hidden; margin-bottom:1.25rem; }
        .rtable { width:100%; border-collapse:collapse; table-layout:fixed; }
        .rtable thead tr { background:#f8fafc; border-bottom:1px solid #e5e7eb; }
        .rtable thead th { padding:0.7rem 1rem; text-align:left; font-size:0.65rem; font-weight:700; text-transform:uppercase; letter-spacing:0.07em; color:#9ca3af; white-space:nowrap; }
        .rtable thead th.tc { text-align:center; }
        .rtable tbody tr { border-bottom:1px solid #f1f5f9; transition:background 0.12s; }
        .rtable tbody tr:last-child { border-bottom:none; }
        .rtable tbody tr:hover { background:#fafbfc; }
        .rtable tbody td { padding:0.85rem 1rem; vertical-align:middle; }
        .rtable tbody td.tc { text-align:center; }

        /* ── MOBILE CARDS ── */
        .mobile-list { display:none; flex-direction:column; gap:0.75rem; margin-bottom:1.25rem; }
        .m-card { background:#fff; border-radius:14px; border:1px solid #e5e7eb; overflow:hidden; }
        .m-card-head { padding:0.85rem 1rem; display:flex; align-items:center; justify-content:space-between; gap:8px; border-bottom:1px solid #f1f5f9; }
        .m-card-body { padding:0.85rem 1rem; display:grid; grid-template-columns:1fr 1fr; gap:0.6rem 1rem; }
        .m-card-foot { padding:0.75rem 1rem; border-top:1px solid #f1f5f9; display:flex; align-items:center; justify-content:space-between; }
        .m-field-label { font-size:0.6rem; font-weight:700; text-transform:uppercase; letter-spacing:0.07em; color:#9ca3af; margin-bottom:2px; }
        .m-field-val   { font-size:0.78rem; font-weight:600; color:#374151; }
        .m-field-sub   { font-size:0.7rem; color:#9ca3af; margin-top:1px; }

        /* ── SHARED ATOMS ── */
        .bk-code { font-size:0.78rem; font-weight:700; color:#111827; font-family:'Courier New',monospace; letter-spacing:-0.01em; }
        .svc-tag { display:inline-block; background:#eff6ff; color:#1e40af; font-size:0.58rem; font-weight:700; padding:1px 6px; border-radius:4px; margin-top:3px; text-transform:uppercase; letter-spacing:0.04em; }
        .cust-av { width:34px; height:34px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:0.72rem; font-weight:700; flex-shrink:0; }
        .cust-name  { font-size:0.82rem; font-weight:600; color:#111827; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
        .cust-email { font-size:0.72rem; color:#9ca3af; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
        .car-name  { font-size:0.82rem; font-weight:600; color:#111827; }
        .car-plate { font-size:0.72rem; color:#9ca3af; font-family:'Courier New',monospace; margin-top:1px; }
        .p-start { font-size:0.78rem; color:#374151; font-weight:500; }
        .p-end   { font-size:0.72rem; color:#9ca3af; margin-top:1px; }
        .price { font-size:0.9rem; font-weight:700; color:#d97706; }

        .badge { display:inline-block; padding:3px 10px; border-radius:20px; font-size:0.62rem; font-weight:700; text-transform:uppercase; letter-spacing:0.05em; white-space:nowrap; }
        ...
        .ab:hover       { border-color:#d97706; background:#fef9c3; } .ab:hover svg      { color:#d97706; }
        .ab.g:hover     { border-color:#22c55e; background:#f0fdf4; } .ab.g:hover svg    { color:#16a34a; }
        .ab.b:hover     { border-color:#3b82f6; background:#eff6ff; } .ab.b:hover svg    { color:#2563eb; }
        .ab.p:hover     { border-color:#8b5cf6; background:#f5f3ff; } .ab.p:hover svg    { color:#7c3aed; }
        .ab.d:hover     { border-color:#ef4444; background:#fef2f2; } .ab.d:hover svg    { color:#ef4444; }

        /* mobile action links */
        .m-action-link { font-size:0.75rem; font-weight:600; text-decoration:none; padding:5px 0; }
        .m-action-link.wf  { color:#16a34a; }
        .m-action-link.dt  { color:#2563eb; }
        .m-action-link.ed  { color:#7c3aed; }
        .m-action-link.dl  { color:#ef4444; background:none; border:none; cursor:pointer; font-family:inherit; }

        .empty-wrap { padding:3.5rem 2rem; text-align:center; }
        .empty-icon { width:56px; height:56px; background:#f3f4f6; border-radius:50%; display:flex; align-items:center; justify-content:center; margin:0 auto 1rem; }

        .pag-wrap { background:#fff; border-radius:14px; border:1px solid #e5e7eb; padding:1rem 1.25rem; }

        /* ── MODAL ── */
        .modal-overlay { position:fixed; inset:0; background:rgba(0,0,0,0.45); z-index:50; display:flex; align-items:center; justify-content:center; opacity:0; pointer-events:none; transition:opacity 0.2s; }
        .modal-overlay.open { opacity:1; pointer-events:all; }
        .modal-box { background:#fff; border-radius:16px; padding:2rem; max-width:370px; width:90%; transform:scale(0.95); transition:transform 0.2s; }
        ...
        .m-cancel  { flex:1; padding:0.7rem; background:#f3f4f6; border:none; border-radius:9px; font-size:0.85rem; font-weight:600; cursor:pointer; font-family:inherit; color:#374151; transition:background 0.15s; }
        .m-cancel:hover { background:#e5e7eb; }
        .m-confirm { flex:1; padding:0.7rem; background:#ef4444; color:#fff; border:none; border-radius:9px; font-size:0.85rem; font-weight:700; cursor:pointer; font-family:inherit; transition:background 0.15s; }
        ...
        @media(max-width:768px){
            .page-hero { padding:1.25rem 1.1rem; border-radius:14px; }
            ...
            .filter-bar { padding:0.75rem; gap:0.6rem; }
            .pills { gap:4px; }
    ...
    <div class="rp">
        ...
        @if(session('success'))
            <div class="toast-ok" id="toastOk">
                ...
                <button onclick="document.getElementById('toastOk').remove()" style="background:none;border:none;cursor:pointer;color:#16a34a;padding:0">
        ...
        <div class="filter-bar">
            <form method="GET" action="{{ route('admin.renter.index') }}" style="flex:1;min-width:160px">
                ...
                <div class="search-wrap">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari booking / customer...">
                    @if(request('search'))
                        <a class="cx" href="{{ route('admin.renter.index', array_filter(['status'=>request('status'),'date_range'=>request('date_range')])) }}">
                            <svg style="width:13px;height:13px" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                            </svg>
                        </a>
            ...
            <div class="pills">
                @php $s = request('status'); $q = array_filter(['search'=>request('search'),'date_range'=>request('date_range')]); @endphp
            ...
            <table class="rtable">
                ...
                <tbody>
                    @forelse($renters as $renter)
                        @php
                            $ini = collect(explode(' ',$renter->user->name))->take(2)->map(fn($w)=>strtoupper($w[0]))->implode('');
                            ...
                            <td colspan="8">
                                <div class="empty-wrap">
                                    ...
                                    <div style="font-size:0.8rem;color:#9ca3af;margin-bottom:1.1rem">
                                        @if(request('search')) Tidak ada penyewaan cocok dengan "{{ request('search') }}"
                                        @elseif(request('status')) Tidak ada penyewaan dengan status tersebut
                                        @else Data penyewaan akan muncul di sini setelah ada pemesanan @endif
                                    </div>
                                    <a href="{{ route('admin.renter.create') }}" class="btn-add">
                    ...
                    <div class="m-card-foot">
                        <div style="display:flex;gap:14px;align-items:center">
                            <a href="{{ route('admin.renter.workflow',$renter->id) }}" class="m-action-link wf">
                                <svg style="width:12px;height:12px;display:inline;margin-right:3px;vertical-align:-1px" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                                Alur Kerja
                            </a>
                            <a href="{{ route('admin.renter.show',$renter->id) }}" class="m-action-link dt">
                                <svg style="width:12px;height:12px;display:inline;margin-right:3px;vertical-align:-1px" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                Detail
                            </a>
