...
<div class="min-h-screen bg-[#F8F7F4] pt-16" x-data="{ activeTab: 'progres' }" x-cloak>
    ...
    <div class="lg:hidden fixed top-0 left-0 right-0 z-50 bg-white border-b border-gray-100 shadow-sm">
        <div class="flex items-center justify-between px-4 h-16">
            <a href="{{ route('bookings.index') }}" class="flex items-center justify-center w-9 h-9 rounded-xl bg-gray-100 hover:bg-gray-200 transition-colors">
            ...
            <button onclick="window.print()" class="flex items-center justify-center w-9 h-9 rounded-xl bg-gray-100 hover:bg-gray-200 transition-colors">
                <svg class="w-5 h-5 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-10">
        ...
        <div class="hidden lg:flex items-center justify-between mb-8">
            <div class="flex items-center gap-4">
                <a href="{{ route('bookings.index') }}"
                   class="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-900 transition-colors font-medium group">
                    <span class="flex items-center justify-center w-8 h-8 rounded-lg bg-white border border-gray-200 group-hover:bg-orange-50 group-hover:border-orange-200 transition-all">
            ...
            <div class="flex items-center gap-2">
                ...
                <button class="inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 rounded-xl text-sm font-semibold text-gray-700 hover:bg-gray-50 hover:shadow-md transition-all">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="hidden lg:flex items-center gap-4 mb-8">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Booking <span class="text-orange-500">#{{ $booking->booking_code }}</span></h1>
                <p class="text-sm text-gray-500 mt-1">Detail informasi dan status booking Anda</p>
            </div>
            <span class="inline-flex items-center px-4 py-2 rounded-full text-sm font-bold {{ $booking->status_badge }} shadow-sm ml-auto">
            ...
            <div class="relative rounded-2xl overflow-hidden h-44 bg-slate-900">
                ...
                <div class="absolute bottom-0 left-0 right-0 p-4">
                    <div class="flex items-end justify-between">
                        <div>
                            <p class="text-white/60 text-xs font-medium mb-0.5">{{ $booking->service_type_label }}</p>
                            <h2 class="text-white text-lg font-bold leading-tight">{{ $booking->car->name ?? 'Toyota Alphard Premium' }}</h2>
                        </div>
                        <span class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-bold {{ $booking->status_badge }} shadow-md">
                            {{ $booking->status_label }}
                        </span>
                    </div>
                </div>
        ...
        <div class="lg:grid lg:grid-cols-3 lg:gap-8">
            ...
            <div class="lg:col-span-2">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        <nav class="grid grid-cols-4">
                            <button @click="activeTab = 'progres'"
                                    :class="activeTab === 'progres' ? 'bg-orange-50 text-orange-600 border-b-2 border-orange-500' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-50 border-b-2 border-transparent'"
                            ...
                            <button @click="activeTab = 'info'"
                                    :class="activeTab === 'info' ? 'bg-orange-50 text-orange-600 border-b-2 border-orange-500' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-50 border-b-2 border-transparent'"
                                    class="flex flex-col items-center gap-1 px-2 py-3 lg:py-4 text-xs lg:text-sm font-semibold transition-all duration-200">
                                <i class="fas fa-circle-info text-base lg:text-lg"></i>
                                <span>Info</span>
                            </button>
                            <button @click="activeTab = 'biaya'"
                                    :class="activeTab === 'biaya' ? 'bg-orange-50 text-orange-600 border-b-2 border-orange-500' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-50 border-b-2 border-transparent'"
                                    class="flex flex-col items-center gap-1 px-2 py-3 lg:py-4 text-xs lg:text-sm font-semibold transition-all duration-200">
                                <i class="fas fa-receipt text-base lg:text-lg"></i>
                                <span>Biaya</span>
                            </button>
                            <button @click="activeTab = 'extension'"
                                    :class="activeTab === 'extension' ? 'bg-orange-50 text-orange-600 border-b-2 border-orange-500' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-50 border-b-2 border-transparent'"
                                    class="flex flex-col items-center gap-1 px-2 py-3 lg:py-4 text-xs lg:text-sm font-semibold transition-all duration-200">
                ...
                <div class="mt-4 space-y-4">
                    ...
                    <div x-show="activeTab === 'biaya'"
                         x-transition:enter="transition ease-out duration-200"
                         x-transition:enter-start="opacity-0 translate-y-2"
                         x-transition:enter-end="opacity-100 translate-y-0">
                        @include('bookings.partials.tab-billing')
                    </div>
                    <div x-show="activeTab === 'extension'"
                         x-transition:enter="transition ease-out duration-200"
                         x-transition:enter-start="opacity-0 translate-y-2"
                         x-transition:enter-end="opacity-100 translate-y-0">
            ...
            <div class="hidden lg:block lg:col-span-1">
                <div class="sticky top-24 space-y-5">
                    ...
                    {{-- CAR CARD --}}
                    <div class="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-3xl shadow-2xl overflow-hidden border border-slate-700/50">
                        <div class="aspect-[16/10] relative overflow-hidden">
                            ...
                            <div class="absolute top-3 right-3">
                                <span class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-bold bg-white/20 backdrop-blur-md text-white border border-white/30">
                        ...
                        <div class="p-5">
                            ...
                            <div class="grid grid-cols-3 gap-2 mb-5">
                                ...
                                <div class="flex flex-col items-center gap-1.5 p-3 bg-white/5 rounded-2xl border border-white/10 hover:bg-white/10 transition-colors">
                                    <i class="{{ $feat[0] }} text-yellow-400 text-sm"></i>
                                    <span class="text-xs text-slate-400 font-medium">{{ $feat[1] }}</span>
                                </div>
                                @endforeach
                            </div>
                            ...
                            <div class="flex items-center gap-3 p-3 bg-white/5 rounded-2xl border border-white/10">
                                <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center flex-shrink-0">
                                    <i class="fas fa-steering-wheel text-white text-sm"></i>
                                </div>
                                <div>
                                    <p class="text-xs text-slate-400">Sopir</p>
                                    <p class="text-sm font-semibold text-white">{{ $booking->driver->name }}</p>
                                </div>
                            </div>
                            @endif
                        </div>
                    ...
                    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-5 space-y-3">
                        ...
                        <div class="space-y-2.5">
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-sm font-semibold text-gray-900">{{ $booking->start_datetime->format('d M Y, H:i') }}</span>
                            </div>
                            <div class="flex items-center justify-between">
                                <span class="text-sm text-gray-500 flex items-center gap-2">
                                    <i class="fas fa-calendar-check text-orange-400 w-4 text-center"></i> Selesai
                                </span>
                                <span class="text-sm font-semibold text-gray-900">{{ $booking->end_datetime->format('d M Y, H:i') }}</span>
                            </div>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-base font-bold text-orange-500">Rp {{ number_format($booking->total_price, 0, ',', '.') }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2.5">
                        ...
                        @endif
                        <button class="w-full flex items-center justify-center gap-2 px-5 py-3.5 bg-gradient-to-r from-orange-400 to-orange-600 text-white rounded-2xl font-bold shadow-lg shadow-orange-200 hover:shadow-orange-300 hover:from-orange-500 hover:to-orange-700 transition-all">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
    ...
    <div class="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-gray-100 px-4 py-3 shadow-2xl">
        <div class="flex gap-3">
            ...
            @endif
            <button class="flex-1 flex items-center justify-center gap-2 py-3 bg-gradient-to-r from-orange-400 to-orange-600 text-white rounded-xl font-bold text-sm shadow-lg shadow-orange-200">
                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M2 3a1 1 0 011-1h2.153a1 1 0 01.986.836l.74 4.435a1 1 0 01-.54 1.06l-1.548.773a11.037 11.037 0 006.105 6.105l.774-1.548a1 1 0 011.059-.54l4.435.74a1 1 0 01.836.986V17a1 1 0 01-1 1h-2C7.82 18 2 12.18 2 5V3z"/>
                </svg>
                Customer Service
...
{{-- EXTEND MODAL --}}
<div id="extendModal" class="hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
        ...
        <form id="extendForm" action="{{ route('bookings.extend', $booking->id) }}" method="POST" class="p-6 space-y-4">
            ...
            <div class="bg-blue-50 border border-blue-100 rounded-2xl p-4 flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm font-bold text-blue-900">{{ $booking->end_datetime->format('d M Y, H:i') }}</p>
                </div>
            </div>
            ...
            <div id="conflictAlert" class="hidden bg-red-50 border border-red-200 rounded-2xl p-4 flex items-start gap-3">
            ...
            <div id="successAlert" class="hidden bg-green-50 border border-green-200 rounded-2xl p-3 flex items-center gap-3">
...
{{-- CANCEL MODAL --}}
<dialog id="cancelModal" class="rounded-3xl shadow-2xl border-0 w-full max-w-md mx-4 backdrop:bg-black/60 backdrop:backdrop-blur-sm">
    <div class="p-6">
        ...
        <form method="POST" action="{{ route('bookings.cancel', $booking) }}">
            ...
            <div class="mb-5">
                ...
                <textarea name="cancellation_reason" rows="4" required
                    class="w-full px-4 py-3 border-2 border-gray-200 rounded-2xl text-sm focus:ring-0 focus:border-red-400 transition-colors resize-none"
                    placeholder="Jelaskan alasan pembatalan..."></textarea>
            </div>
            <div class="flex gap-3">
                <button type="button" onclick="document.getElementById('cancelModal').close()"
                    class="flex-1 py-3 bg-gray-100 hover:bg-gray-200 rounded-2xl font-semibold text-gray-700 transition text-sm">
                    Kembali
                </button>
                <button type="submit"
                    class="flex-1 py-3 bg-red-600 text-white rounded-2xl font-bold hover:bg-red-700 transition text-sm shadow-md shadow-red-200">
                    Konfirmasi Batalkan
                </button>
            </div>
        </form>
...
<script>
    ...
    function closeExtendModal() {
        ...
        document.getElementById('extraHours').textContent = '0 jam';
        document.getElementById('extraPrice').textContent = 'Rp 0';
    }
    ...
    setTimeout(() => {
        window.location.href = window.location.pathname; // redirect clean, bukan reload
    }, 1500);
} else {
    ...
    function showToast(message, type) {
        ...
        toast.className = `fixed top-4 right-4 left-4 sm:left-auto sm:min-w-72 flex items-center gap-3 px-5 py-4 rounded-2xl shadow-xl text-white text-sm font-semibold z-[9999] transition-opacity duration-300 ${type === 'success' ? 'bg-green-500' : 'bg-red-500'}`;
