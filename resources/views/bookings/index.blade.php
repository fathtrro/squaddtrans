        ...
        <div class="bg-white rounded-2xl shadow-md p-6 flex items-center justify-between min-w-[260px]">
            ...
            @if($bookings->count())
                ...
                <div class="bg-white rounded-2xl shadow-md p-4 mb-6">
                    <form method="GET" action="{{ route('bookings.index') }}" class="flex flex-col md:flex-row gap-4">
                        ...
                        <div class="flex gap-2">
                            <button type="button"
                                    onclick="document.getElementById('view-grid').classList.add('bg-gray-200'); document.getElementById('view-list').classList.remove('bg-gray-200');"
                                    id="view-grid"
                            ...
                            <button type="button"
                                    onclick="document.getElementById('view-list').classList.add('bg-gray-200'); document.getElementById('view-grid').classList.remove('bg-gray-200');"
                                    id="view-list"
                                    class="p-2 rounded-lg bg-gray-200 hover:bg-gray-100 transition-colors">
                                <svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="space-y-4">
                    ...
                    <div class="bg-white rounded-2xl shadow-md overflow-hidden hover:shadow-lg transition-all duration-200">
                        <div class="p-6">
                                ...
                                <div class="flex-grow">
                                    <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p class="font-bold text-gray-900 text-sm mb-0.5">{{ $booking->car->name }}</p>
                                            <p class="text-xs text-gray-600">{{ $booking->car->license_plate ?? 'N/A' }}</p>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <div class="flex items-center gap-2">
                                                @if($booking->payments->first())
                                                    @php
                                                        $paymentMethod = $booking->payments->first()->payment_method;
                                                    @endphp
                                                    @if($paymentMethod == 'bank_transfer')
                                                    <svg class="w-4 h-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 14v3m4-3v3m4-3v3M3 21h18M3 10h18M3 7l9-4 9 4M4 10h16v11H4V10z"/>
                                                    </svg>
                                                    <span class="text-sm text-gray-900 font-medium">Bank Transfer</span>
                                                    @elseif($paymentMethod == 'credit_card')
                                                    <svg class="w-4 h-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/>
                                                    </svg>
                                                    <span class="text-sm text-gray-900 font-medium">Credit Card</span>
                                                    @else
                                                    <svg class="w-4 h-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/>
                                                    </svg>
                                                    <span class="text-sm text-gray-900 font-medium">E-Wallet</span>
                                                    @endif
                                                @else
                ...
                <div class="mt-8 flex justify-center">
                    <div class="flex items-center gap-2">
                        ...
                        @else
                            <a href="{{ $bookings->previousPageUrl() }}" class="px-3 py-2 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                                </svg>
                            </a>
                        ...
                        @foreach ($bookings->getUrlRange(1, $bookings->lastPage()) as $page => $url)
                            @if ($page == $bookings->currentPage())
                                <span class="w-10 h-10 rounded-lg bg-amber-500 text-white font-bold flex items-center justify-center">
                                    {{ $page }}
                                </span>
                            @else
                                <a href="{{ $url }}" class="w-10 h-10 rounded-lg border border-gray-200 text-gray-700 font-medium hover:bg-gray-50 transition-colors flex items-center justify-center">
                                    {{ $page }}
                                </a>
                            @endif
                        @endforeach
                        ...
                        @if ($bookings->hasMorePages())
                            <a href="{{ $bookings->nextPageUrl() }}" class="px-3 py-2 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
